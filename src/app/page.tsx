...
export default function SolarPlatformPage() {
  ...
  const calculations = useMemo(() => {
    // Average residential tariff benchmark in India ~ ₹7.5 / kWh
    const unitsPerMonth = monthlyBill / 7.5;
    const dailyUnitsNeeded = unitsPerMonth / 30;

    // 1 kWp system produces approx 4 kWh (units) per day under standard Indian solar insolation
    const rawCapacity = dailyUnitsNeeded / 4;
    let recommendedKW = Math.round(rawCapacity * 2) / 2; // round to nearest 0.5 kW
    if (recommendedKW < 1) recommendedKW = 1;

    // Benchmark cost per kW ~ ₹60,000 to ₹65,000 installed
    ...
    const annualSavings = monthlyBill * 12 * 0.92; // Estimated 92% bill reduction
    const paybackYears = Number((netCost / annualSavings).toFixed(1));
    const requiredRoofSqFt = recommendedKW * 90; // ~90-100 sq.ft per kW
  ...
  return (
    <div className="min-h-screen bg-[#0b0f0e] text-[#f2ede4] antialiased selection:bg-amber-500 selection:text-black">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-12">
        ...
        {/* Interactive ROI & Subsidy Calculator */}
        ...
        <footer className="pt-8 border-t border-white/10 text-xs text-white/50 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-serif font-bold text-white">SuryaMitra</span>
            <span>• Rooftop Solar Advisory & Feasibility Network</span>
          </div>
          <p className="text-[11px]">
            © {new Date().getFullYear()} SuryaMitra. All rights reserved.
          </p>
        </footer>
      </main>
    </div>
